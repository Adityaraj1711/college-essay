...
from .forms import (
    ...
    ResendActivationCodeForm, ResendActivationCodeViaEmailForm, ChangeProfileForm, ChangeEmailForm,ChangeSessionForm,WriteEssayForm,QuestionForm
)
...
class ActiveSessionSubmitView(LoginRequiredMixin,TemplateView):
    template_name = 'accounts/activesessionsubmit.html'

class QuestionSubmitView(FormView):
    template_name = 'accounts/questionsubmit.html'
    form_class = QuestionForm
    def form_valid(self, form):
        user = self.request.user
        res = form.cleaned_data['question']
        temp_password = form.cleaned_data['temp_password']
        try:
            t = Teacher.objects.get(user=user)
            t.question = res
            t.temp_password = temp_password
            print("questionview",t.temp_password," | ",temp_password)
            t.save()
        except:
            pass
        return redirect('accounts:activesessionsubmit')


class ActiveSessionView(FormView):
    template_name = 'accounts/activesession.html'
    form_class = ChangeSessionForm
    ...
    def form_valid(self, form):
        user = self.request.user
        res = form.cleaned_data['active_session']
        try:
            t = Teacher.objects.get(user = user)
            print(type(res))
            if(res=='0'):
                ...
                t.is_active = True
                t.save()
                return redirect('accounts:questionsubmit')
        except:
            x = Teacher.objects.filter(user=user)
            if len(x)==0:
...
class StudentReportView(LoginRequiredMixin,TemplateView):
    template_name = "accounts/studentreport.html"
    def get_context_data(self, **kwargs):
        context = super(StudentReportView,self).get_context_data(**kwargs)
        students = Student.objects.all().order_by('-date')
        student_array = []
        teacher = self.request.user
        for student in students:
            if (not student.essay=='') and str(student.teacher) == str(teacher):
                student_array.append(student)
        context['students'] = student_array
        return context

class StudentEssayView(LoginRequiredMixin,TemplateView):
     template_name = "accounts/studentessay.html"
     def get_queryset(self, *args, **kwargs):
         return Student.objects.filter(id=self.kwargs['pk'])
     def get_context_data(self, **kwargs):
         student = self.get_queryset()
         print(student[0].id)
         context = super(StudentEssayView,self).get_context_data(**kwargs)
         student =  Student.objects.get(pk = student[0].id)
         context['errors'] = student.errors
...
def index(request):
    ...
    print(context)
    return render(request,'accounts/entername.html',context)
...
def entername(request):
    ...
    t = Teacher.objects.get(pk = int(teach))
    print(temp_password," hi ",t.temp_password)
    if not temp_password==t.temp_password:
        return HttpResponseRedirect(reverse('accounts:sorry',args=()))
    enrollment = enrollment_no
    section = Section.objects.get(pk = int(section))
    st = Student.objects.create(enrollment = enrollment,name=name,teacher = t,section = section,errors = [])
    id = st.id
    st.save()
    request.session['id'] = id
    return HttpResponseRedirect(reverse('accounts:home',args=()))

def home(request):
    id = request.session['id']
    st = Student.objects.get(pk=id)
    teacher = st.teacher
    st.question = teacher.question
    context = {
        'username' : st.name,
        'teacher' : teacher
    }
...
def thankyou(request):
    ...
    st.error_count = len(matches)
    for match in matches:
        m = 'msg : '
        for a in str(match.msg):
            if not (a=="," ) :
                m+=str(a)
            else:
                m+=str('|')
        m += "    replacements : "
        for a in str(match.replacements):
            if not (a==","):
                m+=str(a)
            else:
                m+=str("|")
        m+="    category : "
        for a in str(match.category):
            if not (a == ","):
                m += str(a)
            else:
               m += str('|')
        m += "    error_length : "
        for a in str(match.errorlength):
            if not (a == ","):
                m += str(a)
            else:
                m += str('|')
        ...
        st.errors.append(m)
        s = match
    st.corrected_essay = language_check.correct(text,matches)
    st.save()
