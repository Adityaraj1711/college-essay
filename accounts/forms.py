...
class SignUpForm(UserCreationForm):
    ...
    enrollment = forms.CharField(label=_('Enrollment No.'))
    def clean_email(self):
        email = self.cleaned_data['email']
        enroll = self.cleaned_data['enrollment']
        enroll = Student.objects.filter(enrollment = enroll).exists()
        user = User.objects.filter(email__iexact=email).exists()
        if user :
            raise ValidationError(_('You can not use this email address.'))
        if enroll :
            raise ValidationError(_('This enrollment number already exist.'))
...
class WriteEssayForm(forms.Form):
    TEACHER = []
    for a in User.objects.all():
        if a.is_active == True:
            TEACHER.append((a.id, a.username))
    teacher = forms.CharField(label='select teacher', widget=forms.Select(choices=TEACHER))
...
class ChangeSessionForm(forms.Form):
    ar = [(1,"Activate"),(0,"Deactivate")]
    active_session = forms.CharField(label="select session",widget=forms.Select(choices=ar))


class QuestionForm(forms.Form):
    question = forms.CharField(required=True,max_length=50)
    temp_password = forms.CharField(required=True)
